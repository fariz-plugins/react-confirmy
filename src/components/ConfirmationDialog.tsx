...
const defaultStyles: Record<Framework, StyleConfig> = {
  tailwind: {
    ...
    arrow: 'absolute w-3 h-3 rotate-45 bg-white border border-gray-200',
  ...
  bootstrap: {
    ...
    arrow: 'popover-arrow position-absolute bg-white border',
...
export const ConfirmationDialog: React.FC<ConfirmationDialogProps> = ({
  ...
  useEffect(() => {
    ...
    const instance = createPopper(triggerRef.current, dialogRef.current, {
      placement: 'top',
      modifiers: [
        {
          name: 'offset',
  ...
  }, [message, popperInstance]);

  if (!isOpen) return null;

  const getIconColor = () => {
    if (framework === 'tailwind') {
      return type === 'danger' ? 'text-red-500' : type === 'warning' ? 'text-yellow-500' : 'text-blue-500';
    }
    return '';
  };
